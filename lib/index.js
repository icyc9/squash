...
squash.prototype.reserve = Promise.promisify(function (time, date, resolve) {
  ...
  .executeAsync(function (resolve) {
    ...
    alertify.maxLogItems(1).delay(0).log('Looking for open reservations')

    function log (message, time) {
      document.getElementsByClassName("default")[0].innerHTML = message
    }
    ...
    var Reserve = (function () {
      ...
      var requested_time_slot = null

      log('Looking for reservations', new Date())
      changeDate(date)
      ...
      function make_reservation () {
        ...
        for(child in children) {
          ...
          if(element_class.indexOf('rbm_Open') > -1) {
            log('Reservations are available.. Opening reservation.', new Date())
            window.LaunchLockedReserver(window,event,'54', court, window.date, window.time,'2');
            document.getElementById('BookMgrModalOverlay').style.height = 0
            log('Reservations are available.. Opening reservation.', new Date())
            alertify.success('Successfully placed reservation for ' + window.date + ' at ' + window.time, new Date())
